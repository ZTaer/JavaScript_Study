...
var ItemStatus;
(function (ItemStatus) {
    ItemStatus[ItemStatus["ATeam"] = 0] = "ATeam";
    ItemStatus[ItemStatus["Bteam"] = 1] = "Bteam";
})(ItemStatus || (ItemStatus = {}));
/* 组件item蓝图 */
class Component {
    constructor(templateId, hostId, itemId) {
        this.templateElement = document.getElementById(templateId);
        this.hostElement = document.getElementById(hostId);
        const copyElement = document.importNode(this.templateElement.content, true);
        this.element = copyElement.firstElementChild;
...
class ProjectItem extends Component {
    constructor(templateId, hostId, itemId, itemText) {
...
// 渲染目标
function allRender(items) {
    clearAllRender();
    for (let item of items) {
        new ProjectItem("target-list", item.status === ItemStatus.ATeam ? "drag-test-start" : "drag-test-end", item.id, item.text);
    }
}
allRender(items);
/* 非便捷构建拖动 - ( 便捷式构建在下方 ) */
...
const domDropZoneArray = Array.prototype.slice.call(domDropZone); // 转换为数组( 核心 )
